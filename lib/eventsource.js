        ...
        req = (isSecure ? https : http).request(options, function(res) {
            ...
            res.on('data', function (chunk) {
                ...
                while ((index = buf.indexOf("\n\n"))) {
                    var message = buf.slice(0, index + 1);
                    ...
                    parts.forEach(function(part) {
                        if (part.match(/^data:(.+)$/)) {
                            d.data += part.match(/^data:(.+)$/)[1].trim();
                            d.data += "\n";
                        } else if (part.match(/^id:(.+)$/)) {
                            d.id = part.match(/^id:(.+)$/)[1];
                        } else if (part.match(/^event:(.+)$/)) {
                            d.event = part.match(/^event:(.+)$/)[1];
                        }
                    });

                    var data = d.data.replace(/\n$/, '');
                    if (data == '' || (d.event != null && d.event == '')) {
                        return;
                    }
                    if (message.id) lastEventId = message.id;
                    _emit(d.event || 'message', new MessageEvent(data));
                }
            });
        });
